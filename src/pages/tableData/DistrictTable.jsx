...
import { useFetchAllDistricts } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const DistrictTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllDistricts(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Districts</Heading>
                    ...
                    <DataTable>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.districtCode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.districtCode || 'N/A'}</Text></DTd>
                                        <DTd>{row?.districtName || 'N/A'}</DTd>
                                        <DTd>
                                            {row?.lgdCode || 'N/A'}
                                        </DTd>
                                    </DTr>
