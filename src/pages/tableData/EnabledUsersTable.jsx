...
import { useFetchAllEnabledUsers } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const EnabledUsers = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllEnabledUsers(pageNumber, pageSize, debouncedSearchTerm);
  ...
  // --- Render Logic ---
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} mb={6} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Enabled Users</Heading>
                ...
                <DataTableContainer>
                    <DataTable>
                        <DThead>
                            <DTr>
                                <DTh>#</DTh>
                                <DTh>Name of User</DTh>
                                <DTh>Department Name</DTh>
                                <DTh>Role Name</DTh>
                                <DTh>Status</DTh>
                                <DTh>Actions</DTh>
                                <DTh>Change Password</DTh>
                            </DTr>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.usercode}>
                                        {/* Username */}
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.username || 'N/A'}</Text></DTd> 
                                        {/* Name of User */}
                                        <DTd>{row.name || 'N/A'}</DTd>
                                        {/* Department Name */}
                                        <DTd>
                                        ...
                                        {/* Status */}
                                        <DTd>
                                            <Tag size="sm" colorScheme={'green'} variant="subtle">Active</Tag>
                                        </DTd>
                                        ...
                                        <DTd>
                                            <HStack spacing={2}>
                                                ...
                                                <Button size="xs" colorScheme="yellow" variant="outline" onClick={() => onOpenModal(row, 'edit')}>
                                                    Edit
                                                </Button>
                                                <Button size="xs" colorScheme="red" variant="outline" 
                                                    // onClick={() => onOpenModal(row, 'disable')}
                                                >
                                                    Disable
                                                </Button>
                                            </HStack>
                                        ...
                                        <DTd>
                                            <HStack spacing={2}>
                                                {/* Call parent's function on click */}
                                                <Button size="xs" colorScheme="gray" variant="outline" onClick={() => onOpenModal(row, 'change')}>
                                                    Change
                                                </Button>
                                            </HStack>
                                        </DTd>
