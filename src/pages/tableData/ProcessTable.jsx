...
import { useFetchAllProcesses } from '../../hooks/adminActions'; 

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const ProcessTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllProcesses(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Processes</Heading>
                ...
                <DataTableContainer>
                    <DataTable>
                        <DThead>
                            <DTr>
                                <DTh>#</DTh>
                                <DTh>Process Name</DTh>
                                <DTh>Process Description</DTh>
                            </DTr>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.processcode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.processcode || 'N/A'}</Text></DTd>
                                        <DTd>{row.processname || 'N/A'}</DTd>
                                        <DTd>{row.processdescription || 'N/A' }</DTd>
                                    </DTr>
                                ))
                            ) : null}
