...
import { useFetchAllFinancialYear } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const FinancialYearTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllFinancialYear(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Financial Year</Heading>
                ...
                <DataTableContainer>
                    <DataTable>
                        <DThead>
                            <DTr>
                                <DTh>#</DTh>
                                <DTh>Financial year From</DTh>
                                <DTh>Financial Year To</DTh>
                            </DTr>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.financialyearcode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.financialyearcode || 'N/A'}</Text></DTd>
                                        <DTd>{row?.financialYearFrom || 'N/A'}</DTd>
                                        <DTd>
                                            {row?.financialYearTo || 'N/A'}
                                        </DTd>
                                    </DTr>
                                ))
                            ) : null}
