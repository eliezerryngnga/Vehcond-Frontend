...
import { useFetchAllVehiclePart } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const VehiclePartTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllVehiclePart(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Vehicle Parts</Heading>
                    ...
                    <DataTable>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.vehiclePartCode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.vehiclePartCode || 'N/A'}</Text></DTd>
                                        <DTd>{row.vehiclePartDescription || 'N/A'}</DTd>
                                    </DTr>
                                ))
                            ) : null}
...
export default VehiclePartTable;