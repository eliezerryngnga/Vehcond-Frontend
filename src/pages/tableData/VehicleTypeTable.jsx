...
import { useFetchAllVehicleType } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const VehicleTypeTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllVehicleType(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Vehicle Type</Heading>
                    ...
                    <DataTable>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.vehicleTypeCode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.vehicleTypeCode || 'N/A'}</Text></DTd>
                                        <DTd>{row.vehicletypedescription || 'N/A'}</DTd>
                                    </DTr>
                                ))
                            ) : null}
