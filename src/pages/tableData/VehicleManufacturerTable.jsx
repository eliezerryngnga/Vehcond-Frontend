...
import { useFetchAllVehicleManufacturers } from '../../hooks/adminActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"


// Props: onOpenModal (function to call when an action button is clicked)
const VehicleManufacturerTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchAllVehicleManufacturers(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Vehicle Manufacturer</Heading>
                ...
                <DataTableContainer>
                    <DataTable>
                        <DThead>
                            <DTr>
                                <DTh>#</DTh>
                                <DTh>Vehicle Manufacturer Name</DTh>
                        ...
                        <DTbody>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.vehicleManufacturerCode}>
                                        <DTd><Text fontWeight="medium" color="teal.600">{row?.vehicleManufacturerCode || 'N/A'}</Text></DTd>
                                        <DTd>{row.vehicleManufacturerName || 'N/A'}</DTd>
                                    </DTr>
                                ))
                            ) : null}
...
export default VehicleManufacturerTable;