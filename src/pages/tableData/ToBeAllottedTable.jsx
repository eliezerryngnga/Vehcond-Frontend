...
import { useFetchToBeAllotted } from '../../hooks/transportActions'; // Adjust path if needed

//import SearchInput from "../../components/core/SearchInput"

// Props: onOpenModal (function to call when an action button is clicked)
const ToBeAllottedTable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchToBeAllotted(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>To Be Allotted</Heading>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.applicationCode}>
                                        ...
                                        <DTd>{row.departmentName || 'N/A'}</DTd>
                                        ...
                                        <DTd>
                                            <HStack spacing={2}>
                                                ...
                                                <Button size="xs" colorScheme="teal" variant="outline" onClick={() => onOpenModal(row, 'allot')}>
                                                    Allot
                                                </Button>
                                                <Button size="xs" colorScheme="yellow" variant="outline" onClick={() => onOpenModal(row, 'tender')}>
                                                    Tender
                                                </Button>
                                            </HStack>
                                        </DTd>
...
export default ToBeAllottedTable;