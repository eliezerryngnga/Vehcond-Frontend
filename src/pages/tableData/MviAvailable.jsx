...
import { useFetchPendingActionWithMviReport } from '../../hooks/transportActions'; // Adjust path if needed

// Props: onOpenModal (function to call when an action button is clicked)
const MviAvailable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchPendingActionWithMviReport(pageNumber, pageSize, debouncedSearchTerm);
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        <Heading size="md" mb={4} color={textColor}>Pending Approval - With MVI Report</Heading>
