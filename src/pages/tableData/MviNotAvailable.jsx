...
// Import your custom DataTable components
import {
  DataTableContainer,
  DataTable,
  DThead,
  ...
  DTPageSizing,
  DTPagination,
} from '../../components/core/DataTable'; // Adjust path if needed

// Import the specific hook for this table
import { useFetchPendingActionWithoutMviReport } from '../../hooks/transportActions'; // Adjust path if needed

// Props: onOpenModal (function to call when an action button is clicked)
const MviNotAvailable = ({ onOpenModal }) => {
  ...
  const {
    ...
    isFetching,
    isPreviousData
  } = useFetchPendingActionWithoutMviReport(pageNumber, pageSize, debouncedSearchTerm); // Changed hook
  ...
  return (
    <Box p={{ base: 2, md: 4 }} borderWidth="1px" borderRadius="md" bg={containerBg} shadow="sm">
        {/* Changed Title */}
        <Heading size="md" mb={4} color={textColor}>Pending Approval - Without MVI Report</Heading>
                            ...
                            {isFetching && (
                                <DTr>
                                    <DTd colSpan={8} textAlign="center" py={4} position="relative">
                                        <Flex justify="center" align="center" position="absolute" inset={0} bg={useColorModeValue("whiteAlpha.700", "gray.800Alpha.700")}>
                                            <Spinner size="md" />
                                        </Flex>
                                    </DTd>
                                </DTr>
                            ...
                            {!isFetching && fetchedData.length > 0 ? (
                                fetchedData.map((row) => (
                                    <DTr key={row.applicationCode}>
                                        ...
                                        <DTd>
                                            {/* MVI Report is always No for this table */}
                                            <Tag size="sm" colorScheme={'red'} variant="subtle">No</Tag>
                                        </DTd>
                                        <DTd>
                                        ...
                                        <DTd>
                                            <HStack spacing={2}>
                                                ...
                                                <Button size="xs" colorScheme="teal" variant="outline" onClick={() => onOpenModal(row, 'approve')}>
                                                    Approve
                                                </Button>
                                                <Button size="xs" colorScheme="red" variant="outline" onClick={() => onOpenModal(row, 'reject')}>
                                                    Reject
                                                </Button>
                                            </HStack>
                                        </DTd>
...
export default MviNotAvailable;